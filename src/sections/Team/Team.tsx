...
const Team = () => {   
  return (
    <div className={`typography !max-w-[50rem] flex flex-col items-center text-center`} id="team">
      <h2>Join a <strong>global</strong> team</h2>
      <p>
        EclipseExpos is a registered NPO established by youth from over 10 countries and dozens of cities who have experience managing several of the world's largest youth-led organizations.
      </p>
      <div className='my-8'>
        {TeamData.map(
          (row, i) => {
            return (
              <div className='flex flex-row items-center gap-2 mb-2' key={`team-row-${i}`}>
                {row.map(
                  (member, j) => {
                    if (member === '') {
                      return (
                        <div key={`team-row-${i}-member-${j}`} className={`h-16 w-16 brightness-90 hover:brightness-100  transition-all ${GradientDirections[Math.floor(Math.random() * GradientDirections.length)]} ${GradientColours[Math.floor(Math.random() * GradientColours.length)]}`}>
                      ...
                      return (
                        <div key={`team-row-${i}-member-${j}`}>
                          <img src={member} alt={`team-member-${i}-${j}`} className='h-16 w-16 object-cover brightness-90 hover:brightness-100 transition-all' />
                        </div>
                      )
                    }
                  }
      ...
      <p>
        We still have room to grow! We’re expanding our team in all departments, including Operations, Tech, Marketing, Production, and Business Development. If you think your story fits well with ours, reach out using the button below!
      </p>
      <Button onClick={() => (window.open('mailto:hiring@eclipseexpos.org'))} className='mt-8'>Join the team</Button>
    </div>
  )
}
